...
export async function initCommand(options: InitOptions): Promise<void> {
  try {
    ...
    console.log(chalk.bold('🛡️  LLM Guardian - Git Hook Installation'));
    ...
    console.log(`${chalk.dim('Validation:')} ${options.validate ? chalk.cyan('Enabled') : chalk.dim('Disabled')}`);
    console.log('');
    ...
    const hookScript = generateHookScript(options.blocking || false, options.validate || false);
 ...
 * @param validate - Whether hook should run validation (type-check, tests)
 ...
 * - Validation: Runs type-check and tests on applied fixes (if --fix used)
 */
function generateHookScript(blocking: boolean, validate: boolean): string {
...
npx llm-guardian check${validateFlag}
...
if [ $EXIT_CODE -ne 0 ]; then
  echo ""
  echo "⚠️  LLM Guardian found issues in staged files"

${blocking
  ? `  echo "❌ Commit blocked (use --no-verify to skip)"
  exit $EXIT_CODE`
  : `  echo "⚠️  Committing anyway (non-blocking mode)"
  echo "💡 Fix issues or use --blocking mode: llm-guardian init --blocking --force"
  exit 0`
}
fi
